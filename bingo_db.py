...
# ========== CONFIGURATION ==========
# Use Railway volume mount path if available, otherwise default to /data
volume_path = os.getenv("RAILWAY_VOLUME_MOUNT_PATH", "/data")
DB_PATH = os.path.join(volume_path, "bingo.db")
DEPOSIT_BONUS = 10
...
# === TEST PERSISTENCE ===
# This block helps verify that the volume at /data (or the mount path) persists across restarts.
# On first run it writes a file; on subsequent runs it checks if the file already exists.
test_file = os.path.join(volume_path, "test_persistence.txt")
try:
    if os.path.exists(test_file):
        print(f"✅ Test file persists – volume is working!")
    else:
        with open(test_file, "w") as f:
            f.write("This should persist.")
        print(f"✅ Test file written to {test_file}")
except Exception as e:
    print(f"⚠️ Test file error: {e}")
# ================================
...
def init_db():
    ...
    # Load fixed cards if empty (using INSERT OR IGNORE to avoid duplicates)
    c.execute("SELECT COUNT(*) FROM cardboards")
    if c.fetchone()[0] == 0:
        for cid, nums in ALL_CARDS.items():
            c.execute("INSERT OR IGNORE INTO cardboards (id, numbers) VALUES (?, ?)",
                      (cid, json.dumps(nums)))
        conn.commit()
